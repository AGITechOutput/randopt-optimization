# basic_benchmark_ascend.py（开源演示版，昇腾单卡优化版）
# 版本：v3.1-final（昇腾单卡专属）
# 功能：动态分层噪声验证 + 多轮统计、可视化、详细日志
# 适配：昇腾910B，CANN 8.0.RC3，torch_npu 2.1.0
import torch
import torch_npu
import argparse
...
def visualize_noise(model_name, sigma_small, sigma_mid, sigma_large):
    # 需要临时加载模型以获取层数
    model = AutoModelForCausalLM.from_pretrained(model_name, torch_dtype=torch.bfloat16, device_map="npu")
    num_layers = len(model.model.layers)
    s1, s2 = num_layers // 3, 2 * num_layers // 3
    strengths = []
...
if __name__ == "__main__":
    ...
    if not torch_npu.npu.is_available():
        raise RuntimeError("昇腾NPU不可用，请检查驱动和torch_npu")

    print("[INFO] 加载GSM8K测试子集（100条）...")
    dataset = load_dataset("gsm8k", "main", split="test[:100]")
    tokenizer = AutoTokenizer.from_p# basic_benchmark_ascend.py（开源演示版，昇腾单卡优化版）
# 版本：v3.2-final（昇腾单卡专属｜修复tokenizer重复加载｜强化答案匹配）
# 功能：动态分层噪声验证 + 多轮统计、可视化、详细日志
# 适配：昇腾910B，CANN 8.0.RC3，torch_npu 2.1.0
import torch
import torch_npu
...
if __name__ == "__main__":
    ...
    model = AutoModelForCausalLM.from_pretrained(args.model, torch_dtype=torch.bfloat16, device_map="npu")
    model.eval()
    print("[INFO] 计算基线精度...")
    ...
    if args.save_result:
        with open("ascend_benchmark_result.json","w") as f:
            json.dump({"base":base_acc,"opt":opt_acc,"gain":opt_acc-base_acc}, f)
        print("[INFO] 结果已保存至 ascend_benchmark_result.json")
